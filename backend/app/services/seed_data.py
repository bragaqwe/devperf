"""
Seed data v3 — генерирует реалистичные raw-записи:
  GitHubCommit, GitHubPullRequest, GitHubReview,
  JiraIssue, JiraTransition, DailyMetric, PerformanceScore
"""
import random
...
_DEVELOPERS = [
    {"id": 1, "display_name": "Alice Chen",    "github_login": "alice_dev",   "jira_account_id": "alice-jira-id",   "email": "alice@company.io",   "team_id": 1},
    {"id": 2, "display_name": "Bob Müller",    "github_login": "bob_codes",   "jira_account_id": "bob-jira-id",     "email": "bob@company.io",     "team_id": 1},
    {"id": 3, "display_name": "Charlie Singh", "github_login": "charlie_eng", "jira_account_id": "charlie-jira-id", "email": "charlie@company.io", "team_id": 1},
    {"id": 4, "display_name": "Diana Torres",  "github_login": "diana_hacks", "jira_account_id": "diana-jira-id",   "email": "diana@company.io",   "team_id": 2},
    {"id": 5, "display_name": "Evan Park",     "github_login": "evan_build",  "jira_account_id": "evan-jira-id",    "email": "evan@company.io",    "team_id": 2},
]

_PROFILES = {
    1: {"base_sp": 6,  "base_prs": 3, "burnout": False, "trend": "improving"},
    2: {"base_sp": 4,  "base_prs": 2, "burnout": False, "trend": "stable"},
    3: {"base_sp": 8,  "base_prs": 5, "burnout": True,  "trend": "declining"},
    4: {"base_sp": 3,  "base_prs": 2, "burnout": False, "trend": "improving"},
    5: {"base_sp": 2,  "base_prs": 1, "burnout": False, "trend": "stable"},
}
...
_REVIEW_BODIES = [
    ...
    "This looks good, but I'd suggest extracting this into a separate method for clarity.",
    ...
    None,
    "The logic looks correct. One minor: consider using a named constant instead of the magic number.",
...
def _ts(base_dt: datetime, h_min=9, h_max=18) -> datetime:
    """Случайное время в рабочие часы на заданную дату."""
...
def _jira_url(key):               return f"https://company.atlassian.net/browse/{key}"
...
async def run_seed(db: AsyncSession, num_weeks: int = 12):
    ...
    for dev in _DEVELOPERS:
        if not await db.get(Developer, dev["id"]):
            db.add(Developer(**dev))
    await db.flush()
    ...
    # ── Очищаем старые raw-данные и метрики ──────────────────────────────────
    # Jira transitions → issues (FK order)
    ...
    # ── Генерируем данные ─────────────────────────────────────────────────────
    # Глобальные счётчики integer PK
    ...
    for dev_data in _DEVELOPERS:
        ...
        # Список PR-объектов накапливаем для привязки ревью от этого же девелопера
        created_prs = []   # list of (pr_id, pr_number, repo)
        ...
        for week_offset in range(num_weeks, 0, -1):
            ...
            for day_off in range(7):
                ...
                # ── Reviews (от этого девелопера на PR других) ───────────────
                ...
                for _ in range(n_jira_done):
                    ...
                    db.add(JiraIssue(
                        ...
                        project_key=proj,
                        issue_type=itype, summary=summary,
                        status="Done",
                        assignee_account_id=jira_id,
                        priority=random.choice(["High", "Medium", "Low", "Critical"]),
                        story_points=sp_pts,
                        html_url=_jira_url(ikey),
                        created_at=created,
                        updated_at=resolved,
                        resolved_at=resolved,
                        reopen_count=0,
                    ))
                    await db.flush()   # нужен id для FK в transitions

                    # Переходы статусов: In Progress → In Review → Done
                ...
                week_daily.append({
                    ...
                    "code_churn":               round(random.uniform(1.1, 2.5), 2),
                    "pr_review_coverage":       round(random.uniform(0.5, 1.0), 2),
                    "reopen_rate":              round(random.uniform(0, 0.2), 2),
            ...
            act_wk = sum(1 for dm in week_daily if dm["commits_count"] + dm["prs_opened"] > 0)

            delivery    = min((sp_wk / 8) * 50 + (prm_wk / 4) * 30, 100) * week_mult
            quality     = random.uniform(60, 90) * week_mult
            collab      = min(rev_wk / 5, 1.0) * 80 + random.uniform(0, 20)
            consistency = (act_wk / 5) * 100
            overall     = delivery * 0.35 + quality * 0.30 + collab * 0.20 + consistency * 0.15

            if profile["burnout"]:
                after_h = round(random.uniform(0.25, 0.55), 3)
                wkend_r = round(random.uniform(0.15, 0.40), 3)
                b_score = round(after_h * 0.35 + wkend_r * 0.25 + random.uniform(0.1, 0.3), 3)
                b_level = "high" if b_score >= 0.65 else "medium"
            else:
                after_h = round(random.uniform(0.0, 0.15), 3)
                wkend_r = round(random.uniform(0.0, 0.08), 3)
                b_score = round(after_h * 0.35 + wkend_r * 0.25, 3)
                b_level = "low"
            ...
            performance_scores.append({
                ...
                "delivery_score":         round(min(delivery,  100), 2),
                "quality_score":          round(min(quality,   100), 2),
                "collaboration_score":    round(min(collab,    100), 2),
                "consistency_score":      round(consistency,         2),
                "velocity_trend":         round(
                    random.uniform(-0.1, 0.15) +
                    (0.02 if profile["trend"] == "improving" else -0.02), 3
                ),
                "overall_score":          round(min(overall,   100), 2),
                "avg_daily_active_hours": round(random.uniform(6, 12) + (2 if profile["burnout"] else 0), 2),
                "weekend_activity_ratio": wkend_r,
                "after_hours_ratio":      after_h,
                "burnout_risk_score":     min(b_score, 1.0),
...
def generate_weekly_metrics(num_weeks: int = 12):
    """Legacy shim — не используется напрямую в v3."""
    return {
